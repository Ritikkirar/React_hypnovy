...
import { addNotification, editSingleRequest, getMerchantDetailsById, getSingleRequest } from "../../../redux/actions/merchantAction";
...
const EditRequest = ({ token }) => {
  ...
  logger("accept,acceptextracharge", accept, acceptextracharge);
  ...
  // logger("holidayDatesLists", holidayDatesLists)


  // logger("selectedDay", selectedDay)
  // logger("selectedTime", selectedTime)
  const ID = singlerequest && singlerequest.merchant._id;
  // logger("Merchant id",ID)
  ...
  // logger("service_type", service_type);
  // logger("pickuplist", pickupaddresslist)
  // logger("weekly_service", weekly_service)
  // logger("holiday_services", merchantInfo);
  ...
  // logger("singlerequest.pickup", singlerequest && singlerequest.pickup_add_id._id)
  // logger("singlerequest.deposit", singlerequest && singlerequest.deposit_add_id._id)
  ...
  const submitHandler = (e) => {
    ...
    const obj = {
      message: 'Customer has created a new request',
      ID: ID
    }
  ...
  // logger("selectDay state", selectedDay)
